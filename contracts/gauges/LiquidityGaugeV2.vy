...
def __init__(_lp_token: address, _minter: address, _admin: address):
    ...
    self.name = concat("Mobius.fi ", symbol, " Gauge Deposit")
    ...
    self.inflation_rate = MOBI20(crv_token).rate()
    self.future_epoch_time = MOBI20(crv_token).future_epoch_time_write()
