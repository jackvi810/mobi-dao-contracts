...
def __init__(
    ...
    @param _token Fee token address (3MOBI)
...
def ve_for_at(_user: address, _timestamp: uint256) -> uint256:
    """
    @notice Get the veMOBI balance for `_user` at `_timestamp`
    @param _user Address to query balance for
    @param _timestamp Epoch time
    @return uint256 veMOBI balance
    """
...
def checkpoint_total_supply():
    """
    @notice Update the veMOBI total supply checkpoint
...
def claim(_addr: address = msg.sender) -> uint256:
    ...
    @dev Each call to claim look at a maximum of 50 user veMOBI points.
         For accounts with many veMOBI related actions, this function
...
def claim_many(_receivers: address[20]) -> bool:
    ...
    @dev Used to claim for many accounts at once, or to make
         ...
         has significant veMOBI history
...
def burn(_coin: address) -> bool:
    """
    @notice Receive 3MOBI into the contract and trigger a token checkpoint
    @param _coin Address of the coin being received (must be 3MOBI)
    @return bool success
    """
...
def kill_me():
    ...
    @dev Killing transfers the entire 3MOBI balance to the emergency return address
